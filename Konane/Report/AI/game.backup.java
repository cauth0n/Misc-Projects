...
public class Game {
	...
	private long minp1TimePerMove = Integer.MAX_VALUE;
	private long maxp1TimePerMove = Integer.MIN_VALUE;
	private long p1TimePerMove = 0;

	private int moveNumber = 0;

	private long minp2TimePerMove = Integer.MAX_VALUE;
	private long maxp2TimePerMove = Integer.MIN_VALUE;
	private long p2TimePerMove = 0;
	...
	public void loopManager() {
		...
		while (!isGameFinished) {
			...
			while (!p1Move) {
				...
				long p1StartTimer = System.currentTimeMillis();
				p1ValueToUpdate = p1.makeAMove();
				long p1EndTimer = System.currentTimeMillis();
				

				long p1MoveTime = p1EndTimer - p1StartTimer;
				System.out.println(p1MoveTime);
				if (p1MoveTime > maxp1TimePerMove) {
					maxp1TimePerMove = p1MoveTime;
				}
				if (p1MoveTime < minp1TimePerMove) {
			...
			while (!p2Move) {
				...
				long p2StartTimer = System.currentTimeMillis();
				p2ValueToUpdate = p2.makeAMove();
				long p2EndTimer = System.currentTimeMillis();

				long p2MoveTime = p2EndTimer - p2StartTimer;

				if (p2MoveTime > maxp2TimePerMove) {
					maxp2TimePerMove = p2MoveTime;
				}
				if (p2MoveTime < minp2TimePerMove) {
	...
	public long getMinp1TimePerMove() {
		return minp1TimePerMove;
	}

	public long getMaxp1TimePerMove() {
		return maxp1TimePerMove;
	}

	public long getP1TimePerMove() {
		return p1TimePerMove / moveNumber;
	}

	public long getMinp2TimePerMove() {
		return minp2TimePerMove;
	}

	public long getMaxp2TimePerMove() {
		return maxp2TimePerMove;
	}

	public long getP2TimePerMove() {
		return p2TimePerMove / moveNumber;
	}
