...
public class MiniMaxWithAB extends Player {
	...
	public MiniMaxWithAB(String colorValue, int treeDepth) {
		super(colorValue);
	...
	public Move makeAMove() {
		return pruning(copyOfGameBoard);
	}

	public Move pruning(Tile[][] gameState) {
		Tile[][] nextState = gameState;
		int x = maxValue(nextState, Integer.MIN_VALUE, Integer.MAX_VALUE);
	...
	public int maxValue(Tile[][] state, int a, int b) {
		...
		counter++;
		printBoard(state);
		printMoves(constructWhiteMoveSet(state));
		ArrayList<Move> possibleMoves = constructWhiteMoveSet(state);

		int max = Integer.MIN_VALUE;
		for (Move possibleMove : possibleMoves) {
			max = Math.max(max, minValue(result(state, possibleMove), a, b));
	...
	public int minValue(Tile[][] state, int a, int b) {
		...
		for (Move possibleMove : possibleMoves) {
			min = Math.min(min, maxValue(result(state, possibleMove), a, b));
			state = undoMove(state, possibleMove);
			if (min <= a) {
				return min;
			}
			...
			// /omg I literally spent 10 hours on this problem
